...
class FlexsliderTest extends WebTestBase {
  ...
  function testOptionSetCrud() {
    ...
    foreach ($testsets as $name) {
      ...
      $defaultOptionset = Flexslider::create();
      foreach ($defaultOptionset->getOptions() as $key => $value) {
    ...
    $testOptions = $testOptions['valid'];

    // Load one of the test option sets
    $optionset = Flexslider::load($testsets[0]);

    // Change the settings
    $optionset->setOptions($testOptions['set2'] + $optionset->getOptions());
    ...
    foreach ($testOptions['set2'] as $key => $value) {
    ...
    try {
      ...
      $this->fail(t('Caught exception: ',  array('@msg' => $e->getMessage())));
    }
